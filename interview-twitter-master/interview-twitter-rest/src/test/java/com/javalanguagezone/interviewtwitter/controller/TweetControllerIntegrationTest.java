...
public class TweetControllerIntegrationTest extends RestIntegrationTest {
  ...
  public void createTweet_tweetIsSaved() {
    ...
    assertThat(fromDb.getAuthor().getUsername(), equalTo(tweet.getUser().getUsername()));
  }
...
@Test
  ...
  private List<String> extractAuthorNames(TweetDTO[] body) {
    return Arrays.stream(body).map( tweet -> tweet.getUser().getUsername()).distinct().collect(toList());
  }
}
